...
class Homework(BaseModel):
    ...
    answer_attachment: Attachment | None
    answer_content: str
    attachment: Attachment | None
    deadline: datetime = Field(alias="jzsj")
    description: str
    grade_attachment: Attachment | None
    grade_content: str = Field("", alias="pynr")
    grade_time: datetime | None = Field(alias="pysj")
    grade: float | str | None = Field(alias="cj")
    grader_name: str = Field("", alias="jsm")
    number: int = Field(alias="wz")
    start_time: datetime = Field(alias="kssj")
    submit_time: datetime | None = Field(alias="scsj")
    submitted_attachment: Attachment | None
    submitted_content: str
    title: str = Field(alias="bt")

    @staticmethod
    def parse_homework_file(file_div: Tag, type_: AttachmentType) -> Attachment | None:
        fl: Tag | None = file_div.select_one(".txt.fl")
        if fl is None:
            return None
        file_node: Tag | None = fl.select_one("a")
